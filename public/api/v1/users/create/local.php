...
try {
    ...
    /**
     * Checkpoint: Deny certain keywords for username
     */

    if (trim($request->payload()->username)==='admin') {
        throw new BadRequestException(
            'Cannot use admin for username'
        );
    }

    if (trim($request->payload()->firstName)==='admin') {
        throw new BadRequestException(
            'Cannot use admin for first name'
        );
    }

    if (trim($request->payload()->lastName)==='admin') {
        throw new BadRequestException(
            'Cannot use admin for last name'
        );
    }
